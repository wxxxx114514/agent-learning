...
def build_13() -> Notebook:
    ...
    header(
        ...
        "> ⚠️ **本章交付的是「教学型生产化骨架」，不是可直接上线的生产实现。**\n"
        "> 每个部件都手写、可读、可跑通，但都做了刻意简化：内存字典冒充数据库、\n"
        "> 单线程假设、缺少真正的 TTL 与并发原子性。凡是这类地方，正文都标了\n"
        "> 「生产上该用什么」，并布置成练习。\n\n"
        "> 另外注意：**本 Notebook 里手写的 `CircuitBreaker` 与 `runtime.py` 里的那个\n"
        "> 不是同一份实现** —— 这里已经带 `probe_in_flight`（半开只放一个探测），\n"
        "> 框架那份还没有，把做法移植过去正是本章练习 4。别把两者当成同一个类。",
    )
